...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Image upsert vector all request from user:', user.id);
    ...
    // Call backend API
    const response = await fetch(`${cleanBackendUrl}/image/upsert_vector_all`, {
      method: 'POST',
      headers: {
    ...
    console.log('Upsert vector all completed successfully');

    return new Response(JSON.stringify(result), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('Error in image-upsert-vector-all function:', error);
    return new Response(
